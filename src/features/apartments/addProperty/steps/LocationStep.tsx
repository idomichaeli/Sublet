...
export default function LocationStep({ data, onUpdate }: StepProps) {
  ...
  const findNeighborhoodFromCoordinates = (
    latitude: number,
    longitude: number
  ): TelAvivLocation | null => {
    let closestArea: TelAvivLocation | null = null;
    let minDistance = Infinity;

    TEL_AVIV_LOCATIONS.forEach((area) => {
      if (area.coordinates) {
        // Calculate distance using Haversine formula
        const R = 6371; // Earth's radius in kilometers
        const dLat = ((latitude - area.coordinates.latitude) * Math.PI) / 180;
        const dLon = ((longitude - area.coordinates.longitude) * Math.PI) / 180;
        const a =
          Math.sin(dLat / 2) * Math.sin(dLat / 2) +
          Math.cos((area.coordinates.latitude * Math.PI) / 180) *
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      <View style={styles.content}>
        ...
        <View style={styles.addressForm}>
          ...
          {/* Neighborhood */}
          <View style={styles.inputField}>
            <Text style={styles.fieldLabel}>Neighborhood (if applicable)</Text>
            <TextInput
              style={styles.textInput}
